...
import type { Win, WinSize } from '@/types/wins'
...
export function EditWinDialog({ win, open, onOpenChange, onSuccess }: EditWinDialogProps) {
    ...
    useEffect(() => {
        if (win && open) {
            ...
            setWinSize(win.size)
            setSelectedGoalId(win.goal_id || undefined)
        }
    }, [win, open])
    ...
    const handleUpdate = async () => {
        ...
        setSubmitting(true)
        const { error } = await updateWin(win.id, {
            title: title.trim(),
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader onClose={() => onOpenChange(false)}>
                    <div>
                        <DialogTitle>Редактировать победу</DialogTitle>
                        <DialogDescription>
                            Измените данные победы
                        </DialogDescription>
                    </div>
                </DialogHeader>
